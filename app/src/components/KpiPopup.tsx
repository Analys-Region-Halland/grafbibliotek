...
interface Props {
  ...
  onClose: () => void;
  /** Renderas inline i artikelflöde istället för som overlay */
  inline?: boolean;
}
...
export default function KpiPopup({
  ...
  // ── Keyboard + body lock (bara i overlay-läge) ──
  useEffect(() => {
    if (inline) return;
  ...
  // ── Inre innehåll (delat mellan overlay och inline) ──
  const toolbarAndChart = (
    <>
        {/* ═══════════ VERKTYGSFÄLT ═══════════ */}
  ...
  // ── Inline-läge: renderas direkt i artikelflöde ──
  if (inline) {
    return (
      <div className="bg-white rounded-none sm:rounded-2xl border-y sm:border border-neutral-200/70
                      shadow-none sm:shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex flex-col relative">
        {toolbarAndChart}
      </div>
    );
  }

  // ── Overlay-läge (standard) ──
  return (
    <div
